...
const NOW = Date.now()
const URL = 'https://fitnessprogramer.com/wp-content/uploads/2021/02/HiP-ABDUCTION-MACHINE.gif'
try {
  const { rowCount } = await client.query(`
    ...
    WHERE store='exercises' AND id='seed-hip-abduction'
      AND (data->>'deleted')::boolean IS NOT TRUE
  `, [
    JSON.stringify({ type:'gif', mime:'image/gif', aspectRatio:1, sizeBytes:0, importedAt:NOW, url:URL }),
    NOW
  ])
  console.log(`✅ Machine abducteurs → image machine (${rowCount} ligne(s))`)
